...
class Users extends Model{
  ...
  public static function getUserProfiles($userId = false){
    ...
    $query = 'SELECT up.profile_id, pd.name, pd.surname, pd.mobile_phone FROM user_profiles up, profile_details pd WHERE up.userId=:userId AND pd.profile_id IN (SELECT profile_id FROM user_profiles) ORDER BY up.profile_id ASC';
    $stm = self::$db->prepare($query);
    $stm->execute(array('userId'=>$userId));
    return $stm->fetchAll(PDO::FETCH_ASSOC);
  }
